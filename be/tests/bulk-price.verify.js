...
async function runFullVerification() {
    ...
    try {
        ...
        const testPreview = async (type, val, rounding, label) => {
            const result = await callController(BulkProductController.preview, {
        ...
        // Descuento excesivo
        const err1 = await callController(BulkProductController.preview, {
            body: { productIds: [productIds[0]], adjustmentType: 'percentage', adjustmentValue: -95 }
        });
        if (err1.data?.data?.[0]?.isValid === false)
            console.log('  ✅ [LÍMITE] Bloqueo descuento > 90%: OK');
        else
            console.log('  ❌ [LÍMITE] No bloqueó descuento excesivo');
        // Precio Negativo
        const err2 = await callController(BulkProductController.preview, {
            body: { productIds: [productIds[0]], adjustmentType: 'fixed', adjustmentValue: -9999999 }
        });
        if (err2.data?.data?.[0]?.isValid === false)
            console.log('  ✅ [LÍMITE] Bloqueo precio <= $0: OK');
        else
            console.log('  ❌ [LÍMITE] No bloqueó precio negativo');
        // Errores de input
        const err3 = await callController(BulkProductController.preview, {
            body: { productIds: [], adjustmentType: 'percentage', adjustmentValue: 10 }
        });
        if (err3.error)
            console.log('  ✅ [INPUT] Validación Array Vacío: OK');
        // --- GRUPO 3: APLICACIÓN Y AUDITORÍA ---
        console.log('\n💾 GRUPO 3: APLICACIÓN REAL Y AUDITORÍA');
        const appResult = await callController(BulkProductController.apply, {
        ...
        if (appResult.status === 200) {
            ...
            if (batch)
                console.log('  ✅ [DB] PrecioChangeBatch persistido');
            const audit = await em2.findOne(AuditLog, { action: 'BULK_PRICE_UPDATE' }, { orderBy: { createdAt: 'DESC' } });
            if (audit && audit.details.batchId === batchId)
                console.log('  ✅ [DB] AuditLog generado con trazabilidad');
            ...
            const rollResult = await callController(BulkProductController.rollback, {
            ...
            if (rollResult.status === 200) {
                ...
                if (revertedPrice !== currentPrice)
                    console.log('  ✅ [ROLLBACK] El precio volvió al estado anterior');
                const batchReverted = await em3.findOne(PriceChangeBatch, { id: batchId });
                if (batchReverted?.isReverted)
                    console.log('  ✅ [ROLLBACK] Lote marcado como revertido');
            }
            else {
        ...
        // --- GRUPO 5: HISTORIAL ---
        console.log('\n📜 GRUPO 5: HISTORIAL');
        const histResult = await callController(BulkProductController.getHistory, {});
        if (histResult.status === 200)
            console.log('  ✅ [HISTORY] Obtención de historial: OK');
...
runFullVerification();
//# sourceMappingURL=bulk-price.verify.js.map