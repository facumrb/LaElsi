...
export class OrdersListComponent {
    ...
    getNextStates(order: IApiOrder): OrderState[] {
        ...
        if (order.deliveryMethod === DeliveryMethod.ENVIO) {
            switch (order.status) {
                case OrderState.PENDING: return [OrderState.PAID];
                case OrderState.PAID: return [OrderState.SHIPPED];
                case OrderState.SHIPPED: return [OrderState.DELIVERED];
                default: return [];
            }
        } else {
            switch (order.status) {
                case OrderState.PENDING: return [OrderState.PAID];
                case OrderState.PAID: return [OrderState.DELIVERED];
                default: return [];
            }
        }
    }
